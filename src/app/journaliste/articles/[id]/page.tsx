...
export default function EditJournalistArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="lp-container max-w-5xl py-10"><h1 className="text-3xl font-bold">Modifier la publication</h1><p className="mt-2 text-sm text-lp-gray">Statut : {STATUS_LABELS[status] || status}</p>{status === "REFUSE" && reason && <div className="my-6 rounded border border-red-200 bg-red-50 p-4 text-sm text-red-900"><strong>Motif du refus :</strong> {reason}</div>}<div className="mt-8"><ArticleForm mode="edit" articleId={articleId} initialValues={initialValues} apiBase="/api/journaliste/articles" returnPath="/journaliste/articles" /></div></div>;
}
