...
export function UserActions({ id, isActive, name, email }: { id: string; isActive: boolean; name: string; email: string }) {
  ...
  return <div className="flex flex-wrap gap-2"><button type="button" disabled={loading} onClick={() => setEditing((open) => !open)} className="rounded-lg px-2 py-1 text-xs font-semibold text-lp-accent hover:bg-lp-accent-soft">Modifier</button><button type="button" disabled={loading} onClick={() => request("PATCH")} className="rounded-lg px-2 py-1 text-xs font-semibold text-lp-accent hover:bg-lp-accent-soft">{isActive ? "Désactiver" : "Réactiver"}</button><button type="button" disabled={loading} onClick={() => request("DELETE")} className="rounded-lg px-2 py-1 text-xs font-semibold text-red-700 hover:bg-red-50">Supprimer</button>{editing && <div className="fixed inset-0 z-50 grid place-items-center bg-[#101828]/55 p-4 backdrop-blur-sm"><div className="w-full max-w-md rounded-2xl border border-white/70 bg-white p-6 shadow-2xl"><h2 className="font-bold">Modifier le compte</h2><p className="mt-1 text-sm text-lp-gray">Les changements sont appliqués immédiatement.</p><input value={values.name} onChange={(e) => setValues({ ...values, name: e.target.value })} placeholder="Nom" className="lp-form-input mt-5 text-sm" /><input value={values.email} onChange={(e) => setValues({ ...values, email: e.target.value })} placeholder="E-mail" type="email" className="lp-form-input mt-3 text-sm" /><div className="mt-5 flex gap-3"><button type="button" disabled={loading} onClick={save} className="lp-btn-accent px-4 py-2 text-sm">{loading ? "Enregistrement…" : "Enregistrer"}</button><button type="button" onClick={() => setEditing(false)} className="rounded-xl border px-4 py-2 text-sm font-semibold hover:bg-gray-50">Annuler</button></div></div></div>}</div>;
}
